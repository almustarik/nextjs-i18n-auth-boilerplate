...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { id } = params
    const notificationIndex = mockNotifications.findIndex((n) => n.id === id && n.userId === session.user.id)

    if (notificationIndex === -1) {
      return NextResponse.json<ApiResult<null>>({ success: false, error: "Notification not found" }, { status: 404 })
    }
    ...
    return NextResponse.json<ApiResult<(typeof mockNotifications)[notificationIndex]>>({
      success: true,
